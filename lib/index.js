...
var VnToday = class {
  ...
  async run() {
    ...
    if (this.method === "vn") {
      ...
      for (const v of res) {
        ...
        str += await this.polishText(0 /* Vn */, v["released"], v["alttitle"] || v["title"]);
      }
      if (str === "那年今天，这些作品发布了...\n") {
      ...
      for (const v of res) {
        ...
        str += await this.polishText(1 /* Cha */, v["vns"][0]["alttitle"] || v["vns"][0]["title"], v["original"] || v["name"]);
      }
  ...
  async polishText(_unique, ..._info) {
    let index = 0;
    let x = [];
    switch (_unique) {
      case 0 /* Vn */:
