...
async function downloadAssets(_ctx) {
  const res = [];
  const ele = import_path3.default.join(__dirname, "elements");
...
async function apply(ctx) {
  ...
  load(ctx);
  const resp = await downloadAssets(ctx);
  ctx.logger.success(resp);
  if (resp.includes("失败")) {
    ctx.logger.error("有资源初始化失败，请尝试重新加载插件以重试！");
  }
